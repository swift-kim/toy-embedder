...
# Downloads the latest engine binaries for use by the linker.
def main():
  ...
  request = requests.get('%s/latest' % github_url)
  ...
  for filename in names:
    ...
    print('Downloading libflutter_engine.so for %s...' % arch)

    download_path = engine_dir / arch
    download_url = '%s/download/%s/%s' % (github_url, version, filename)
